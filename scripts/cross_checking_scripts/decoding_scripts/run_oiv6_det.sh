...
if [[ ${RUN} == "gnu_parallel" ]]; then
    ...
    run_scripts () {
        sbatch --gpus 0 --reservation=deepvideo2 --job-name=oiv6_det_decode $1
        # grid batch --job-name=oiv6_det_decode --reservation=deepvideo2 --nodelist=kopspgd16p --cpus 2 -- bash $1
    }
    export -f run_scripts
else
    run_scripts () {
...
# DECODE + NN_PART2
for QP_DIR in $( find ${BITSTREAM_DIR} -type f -name "mpeg-oiv6-detection*.bin"  | xargs dirname | sort | uniq );
do
    echo $QP_DIR
