...
def generate_yuv_clip(sequence, src_seq, width, height, frames_to_be_encoded, frame_skip, bit_depth):
    ...
    os.makedirs(imgs_subfolder, exist_ok=True) # Needed for CompressAI-Vision folder presence check
...
def main():
    for sequence, params in SEQ_DICT.items():
        ...
        src_seq, total_frames, frame_rate, bit_depth, frame_skip, frames_to_be_encoded = params

        generate_yuv_clip(sequence, src_seq, width, height, frames_to_be_encoded, frame_skip, bit_depth)
