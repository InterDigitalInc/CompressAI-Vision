...
if [[ ${RUN} == "gnu_parallel" ]]; then
    ...
    run_scripts () {
        sbatch -p longq7 --mem=32G -c 4 --job-name=oiv6_det $1
    }
    export -f run_scripts
else
    run_scripts () {
...
for QP in ${QPS}
do
    run_scripts "../gridfiles/mpeg_cfp_oiv6_detection.sh ${INPUT_DIR} ${BITSTREAM_DIR} ${EXPERIMENT} ${DEVICE} ${QP} ${config_name}"
done
...
if [[ ${RUN} == "gnu_parallel" ]]; then
    ...
    sbatch --dependency=singleton --job-name=oiv6_det  gen_csv.sh OIV6 ${BITSTREAM_DIR}/split-inference-image/cfp_codec${EXPERIMENT}/MPEGOIV6/mpeg-oiv6-detection
else
    bash gen_csv.sh OIV6 ${BITSTREAM_DIR}/split-inference-image/cfp_codec${EXPERIMENT}/MPEGOIV6/mpeg-oiv6-detection
fi
