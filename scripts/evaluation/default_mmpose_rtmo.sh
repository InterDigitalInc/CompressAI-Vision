...
# COCO 2017 Val - Pose Estimation with RTMO (Real-Time Multi Person Pose Estimation)
# Bottom-up method
...
${ENTRY_CMD} --config-name=${CONF_NAME}.yaml \
             ...
             ++vision_model.arch=rtmo_multi_person_pose_estimation \
             ++dataset.type=MMPOSEDataset \
             ++dataset.settings.patch_size="[640, 640]" \
             ++dataset.datacatalog=COCO \
             ++dataset.config.root=${COCO_2017_VAL_SRC} \
             ++dataset.config.annotation_file=annotations/person_keypoints_val2017.json \
             ++dataset.config.imgs_folder=val2017 \
             ++dataset.config.dataset_name=coco2017val \
             ++evaluator.type=MMPOSE-COCO-EVAL \
