...
def main(p):
    ...
    assert(p.dir is not None), "please specify OpenImageV6 source directory for images (and masks)"
    if pathExists(p.target_dir):
        print("FATAL: target directory %s exists already. Please remove it first"
            % (p.target_dir))
        return
    assert(pathExists(p.dir)), "directory "+p.dir+" does not exist"
    assert(p.label is not None), "please provide image level labels --label ('detection_validation_labels_5k.csv')"
