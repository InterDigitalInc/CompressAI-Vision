...
def calc_complexity_nn_part2_dn53(vision_model, dec_features):
    ...
    if isinstance(dec_features['data'][0], list): # image task
        ...
        x = dec_features['data']
        x = {k: v.unsqueeze(0).to(device=device) for k, v in zip(vision_model.split_layer_list, x.values()) }
...
def calc_complexity_nn_part2_plyr(vision_model, data, dec_features):
    ...
    macs_sum = 0
    for partial_model, input_res, input_constructure in zip(partial_model_lst, input_res_list, input_constructure_lst):
...
def measure_mac(partial_model, input_res, input_constructor):
    macs, params = get_model_complexity_info(partial_model, 
                                            input_res=input_res, 
...
def prepare_roi_head_input_fpn(resolutions):
    ...
    feature_dict = {f'p{e+2}':feature for e, feature in enumerate(feature_lst)}
    return dict(images=resized_img, features=feature_dict, proposals=proposals, targets=None)
